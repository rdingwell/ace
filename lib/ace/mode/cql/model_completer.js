...
"use strict";
  ...
  var quotedIndentifierInserter = function(aceToken, pos){
     ...
     this.insertMatch = function(editor,data){
      ...
      // does it end in a "
      if(this.token.value[0] == '"' && this.token.value[this.token.value.length-1] != '"'){
        // this is problimatic as we cant be sure where to end this 
      }
      if(this.token.value[0] == '"' ){

      }
      if(this.token.value.trim() == ""){
        if(v.indexOf("\"") == 0){
          this.editor.session.remove(range);
          this.editor.execCommand("insertstring",v);
        }
        else{
          //remove the whitesapce from the center
        }
      }
      else{
      ...
      if(rule ){
        ...
        var contextVars = context.getVaribleHierarchy().map(function(cv){
                if(cv.type && cv.type.type)
                 return {name: cv.id, value: cv.id, score: 1000, meta: cv.type.type}
  ...
  var cqlModelCompleter = {
    getCompletions: function(editor, session, pos, prefix, callback) {
      ...
      try{
        ...
        // all kinds of errors can happen in the retreieve completer if there are not alot of tokens 
        //available
