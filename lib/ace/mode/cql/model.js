define(function(require, exports, module) {
  ...
  CompositeType.prototype.getPropertyNames = function() {
    var names = [];
     for(var i in this.properties){
      names.push(i)
     }
  ...
  Model.prototype.resolve = function(type) {
    ...
    else if(type.indexOf(this.name) == 0){
      //strip off the name and the dot
      return this.types[type.substring(this.name.length)];
    } // maybe this model does not fully qualify names so lets look there first 
     // then try to fully qualify it then try for a system base type
    else {
      return this.types[type] || this.types[this.name+"."+type] || system_types.resolve("System."+type)
    }
