define(function(require, exports, module) {
  ...
  var getTypeHierarchy = function(prefix) {
    var hier = [];
    var self = this;
    this.getPropertyNames().forEach(function(p){
      var selfPrefix = prefix+"."+p
      hier.push(selfPrefix);
      var prop = self.resolve(p)
      if(prop){hier.concat(p.getTypeHierarchy(selfPrefix))}
    })
    return hier;
  }
  ...
  var SimpleType = function(type){
    ...
    this.getPropertyNames = function(){return []}
    this.properties = {}
  }
  ...
  ListType.prototype.resolve = function(prop) {
    this._type.resolve(prop)
  };
  
  ListType.prototype.resolvePossibles = function() {
    this._type.resolvePossibles()
  };

  ListType.prototype.getPropertyNames = function() {
    this._type.getPropertyNames()
  };
  ...
  var Model = function(model){
    ...
    this.types = {};
    var self = this;
    (model.types || []).forEach(function(v){
      self.types[v.type] = new ModelType(v)
    });
  ...
  Model.prototype.resolve = function(type) {
    ...
    if(listType){
      ...
      return new systemTypes.ListType(t)
    }
    return this.types[type] || system_types.resolve(type)
  };
  ...
  ModelType.prototype.resolve = function(prop) {
    ...
    return prop ? model.resolve(prop.type) : null
  };
  
  ModelType.prototype.propertyNames = function() {
     var names = this.properties.getOwnPropertyNames()
     return getBaseType() ? names.concat(getBaseType.propertyNames()) : names
  };
 ...
 exports.System = new SystemTypes()
