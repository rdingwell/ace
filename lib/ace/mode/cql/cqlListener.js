...
cqlListener.prototype.exitQuerySource = function(ctx) {
  if(ctx.retrieve()){
    ...
    ctx.__type = this.resolveQI(ctx.qualifiedIdentifier(), this.currentContext)
...
cqlListener.prototype.exitAliasedQuerySource = function(ctx) {
  ...
  ctx.__alias = ctx.alias().identifier().__indentifier
...
cqlListener.prototype.enterQueryInclusionClause = function(ctx) {
  ...
  this.currentContext = this.currentContext.createChildContext(cts.start,ctx.stop)
};
...
cqlListener.prototype.exitRetrieve = function(ctx) {
  ...
  var model = ctx.modelIdentifier().__indentifier;
  var klass = ctx.namedTypeSpecifier().__indentifier;
  ctx.type = {} ; // should be model class from ctx.namedTypeSpecifier()
};
...
cqlListener.prototype.exitQuery = function(ctx) {
  ...
  if (ctx.returnClause()){
   listType = ctx.returnClause().__type 
    ...
    listType = {}// make into tuple type with aliases ctx.__aliases; // this will be a tuple of all the aliased q sources and defined vars
...
cqlListener.prototype.exitDefineClauseItem = function(ctx) {
  this.currentContext.set(ctx.identifier().__indentifier,ctx.expression().__type)
};
...
cqlListener.prototype.exitDurationBetweenExpression = function(ctx) {
  ctx.__type = {} // duration , whatever that is
};
...
cqlListener.prototype.exitRetrieveExpression = function(ctx) {
  ctx.__type = this.resolveModelType(ctx)
};
...
cqlListener.prototype.exitNotExpression = function(ctx) {
  ctx.__type = {} // boolean type or uncertainty type
};
...
cqlListener.prototype.exitIdentifier = function(ctx) {
  ctx.__indentifier = ctx.QUOTEDIDENTIFIER() || ctx.INDENTIFIER();
};
