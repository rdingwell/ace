...
function cqlListener() {
  ...
  var includes = {};// just the name and alias for now.  SHould be expandable to pull in other info from lib 
  var models = [];  // just model names 
  var paramteters = {}; // just name, maybe type in the meta
  var functions = {}; // name, params?
  var expressions = []; // just identifiers ?
  var valuesets = []; // should be indentified as a valueset
	return this;
}
...
cqlListener.prototype.exitUsingDefinition = function(ctx) {
   this.models.push(ctx.identifier());
};
...
cqlListener.prototype.exitIncludeDefinition = function(ctx) {
   this.includes[ctx.identifier()] = (ctx.localIdentifier() || ctx.identifier()) ;
};
...
cqlListener.prototype.exitParameterDefinition = function(ctx) {
   this.paramteters[ctx.identifier()] = ctx.typeSpecifier();
};
...
cqlListener.prototype.exitValuesetDefinition = function(ctx) {
  this.valuesets.push(ctx.identifier());
};
...
cqlListener.prototype.exitExpressionDefinition = function(ctx) {
  this.expressions.push(ctx.identifier());
};
...
cqlListener.prototype.exitFunctionDefinition = function(ctx) {
  this.functions[ctx.identifier()] = ctx.operandDefinition().map(function(o){return [o.identifier(),o.typeSpecifier()]});
};
